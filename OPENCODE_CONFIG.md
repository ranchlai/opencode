...
> **Human docs:** https://opencode.ai/docs
...
`loop.verify`: shell commands that must exit `0` before autonomous `/loop` accepts `LOOP_DONE`. Run from project directory.
...
6. **Permissions:** `"deny"` hard-blocks; `"ask"` prompts the user; `"allow"` is silent. Prefer `"ask"` for destructive shell when unsure.
...
12. When documenting for humans, link https://opencode.ai/docs — this file is for machine configuration.
