...
OpenCode remembers tool calls that failed in the current project and warns the agent if it retries the same call.

What counts as a failure:

- The tool throws an error
- A tool returns a nonzero `exit` code in its metadata (for example `bash`)

When the same tool and arguments fail again later, OpenCode prepends a warning to the tool output:
...
- Memory is **project-scoped** and stored in the local SQLite database (`memory` table).
- Matching uses a hash of the tool name plus arguments, so a changed command or input is treated as new.
- A later **successful** call with the same arguments clears that failure entry.
- This is a soft gate: the tool still runs; the warning steers the model away from repeating a known failure.
- Useful with [`/loop`](/docs/loop) so long runs do not thrash on the same broken command.
