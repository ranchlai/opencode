...
If you omit the filename, OpenCode prompts for one.

---

### save_and_quit

Save the last model response to a file and exit OpenCode.

```bash frame="none"
/save_and_quit notes.md
```

If you omit the filename, OpenCode prompts for one, then exits after a successful save.
...
Run a long-horizon goal autonomously until done, blocked, or a budget expires. [Learn more](/docs/loop).

```bash frame="none"
/loop 2h ship the feature
/loop stop
```

Optional budgets: a duration (`2h`, `30m`) and/or a round count (`50`). Configure `loop.verify` in [config](/docs/config#loop) so `LOOP_DONE` only sticks when your checks pass.
...
Review changed code for reuse, quality, efficiency, and altitude, then apply high-confidence cleanup. Does not hunt for correctness bugs — use `/review` for that.
...
With no arguments, this uses uncommitted changes. Pass a path, commit, branch, or PR to narrow the target.
