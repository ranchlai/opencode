...
Configure autonomous [`/loop`](/docs/loop) behavior.
...
- `verify` - Shell commands that must exit `0` before `LOOP_DONE` is accepted. Commands run from the project directory in order. If a command fails, the loop continues with the failure output. `LOOP_BLOCKED` skips verification.

[Learn more about `/loop`](/docs/loop).
