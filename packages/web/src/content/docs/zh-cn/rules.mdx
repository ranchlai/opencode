...
如果您已在使用 Cursor，也可以继续把规则放在 `.cursor/rules/*.mdc` — opencode 会自动发现并应用它们。详见 [Cursor 规则](#cursor-规则-mdc)。
...
2. **Cursor 规则**，位于 `.cursor/rules/**/*.mdc`（见 [Cursor 规则](#cursor-规则-mdc)）
3. **全局文件**，位于 `~/.config/opencode/AGENTS.md`
4. **Claude Code 文件**，位于 `~/.claude/CLAUDE.md`（除非已禁用）

对于 `AGENTS.md` / `CLAUDE.md`，在每个类别中第一个匹配的文件优先。例如，如果您同时拥有 `AGENTS.md` 和 `CLAUDE.md`，则只会使用 `AGENTS.md`。同样，`~/.config/opencode/AGENTS.md` 优先于 `~/.claude/CLAUDE.md`。Cursor 的 `.mdc` 规则会在这些文件之外额外加载，并按其 frontmatter 模式生效。

---

## Cursor 规则 (`.mdc`)

opencode 会自动发现 `.cursor/rules/**/*.mdc` 中的 Cursor 项目规则，并根据 frontmatter 应用它们，行为与 Cursor 一致。
...
| 字段 | 类型 | 作用 |
| :----------- | :-------------- | :-------------------------------------------------- |
| `alwaysApply` | `boolean` | 为 `true` 时，每个会话都会注入该规则 |
| `globs` | `string` 或 `string[]` | 读取匹配文件时自动附加 |
| `description` | `string` | 任务相关时由 agent 自行 Read 加载 |

字段如何组合：

| `alwaysApply` | `description` | `globs` | 行为 |
| :------------ | :------------ | :------- | :--------------------------------------------------------------- |
| `true` | — | — | 始终注入，忽略 globs 与 description |
| `false` | — | 有值 | 读取匹配文件时自动附加 |
| `false` | 有值 | 省略 | 列入可用规则列表，agent 在相关时 Read |
| `false` | 省略 | 省略 | 手动 — 用 `@` 提及或显式 Read |

说明：

- 仅自动发现 `.cursor/rules/` 下的 `.mdc` 文件。其中的普通 `.md` 文件会被忽略，除非你通过 `instructions` 添加。
- 注入提示词前会去掉 frontmatter，只保留正文。
- 也会扫描工作目录到仓库根之间的嵌套 `.cursor/rules/`。
- **不需要**在 `opencode.json` 里写 `.cursor/rules/*.mdc` — 会自动发现。若你仍写了，opencode 会按 frontmatter 模式处理且不会重复加载。
